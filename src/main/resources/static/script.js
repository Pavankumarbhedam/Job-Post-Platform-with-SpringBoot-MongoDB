...
async function fetchJobs() {
  //  console.log("Fetching jobs..."); // Debugging line to check if this is being called
    ...
    try {
        const response = await fetch("http://localhost:8080/posts"); // Update the backend API endpoint if needed
...
async function addJob() {
    ...
    if (profile && des && exp && tech.length > 0) {
        ...
        try {
            await fetch("http://localhost:8080/addJobPost", {
                method: "POST",
...
async function searchJobs() {
    ...
    try {
        const response = await fetch(`http://localhost:8080/posts/search/${encodeURIComponent(searchTerm)}`);
        const jobs = await response.json();
