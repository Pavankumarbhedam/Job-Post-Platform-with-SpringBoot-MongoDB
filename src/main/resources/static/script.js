...
async function fetchJobs() {
  //  console.log("Fetching jobs..."); // Debugging line to check if this is being called
    ...
    try {
        const response = await fetch("https://your-backend-service.onrender.com/posts"); // Update the backend API endpoint if needed
...
async function addJob() {
    ...
    if (profile && des && exp && tech.length > 0) {
        ...
        try {
            await fetch("https://your-backend-service.onrender.com/addJobPost", {
                method: "POST",
...
async function searchJobs() {
    ...
    try {
        const response = await fetch(`https://your-backend-service.onrender.com/posts/search/${encodeURIComponent(searchTerm)}`);
        const jobs = await response.json();
